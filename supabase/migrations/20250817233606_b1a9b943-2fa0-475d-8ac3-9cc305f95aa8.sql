...
CREATE INDEX IF NOT EXISTS idx_user_purchases_user_product ON public.user_purchases(user_id, product_id);

-- Digital product files index
CREATE INDEX IF NOT EXISTS idx_digital_product_files_product_id ON public.digital_product_files(product_id);

-- Courses partial index for active courses
CREATE INDEX IF NOT EXISTS idx_courses_active ON public.courses(id) WHERE is_active = true;
...
CREATE INDEX IF NOT EXISTS idx_ratings_content_type_id ON public.ratings(content_type, content_id);
...
-- Ensure trigger exists for discussion replies counter
...
-- Ensure trigger exists for comment likes counter
