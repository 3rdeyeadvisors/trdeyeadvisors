-- Ensure all requested products are properly deactivated
...
WHERE 
  ...
  LOWER(title) LIKE '%shirt%system%' OR
  LOWER(title) LIKE '%rewrite%system%' OR
  LOWER(title) LIKE '%system%rewrite%';