-- Add missing foreign key constraints and performance indexes (checking existing first)

-- Check and add foreign key constraints that don't exist
DO $$
BEGIN
...
-- Add unique constraints where needed (checking if they exist first)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
